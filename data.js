window.LOYALTY_DATA = {
  "sourceFile": "/Users/steven/Desktop/LoyaltyHub Product Map 0623.xlsx",
  "sheet": "Loyalty Hub Products Map",
  "updated": "2026-06-23",
  "hierarchy": [
    {
      "level": "L1",
      "name": "Supply Chain product",
      "description": "E.g. lounge, limo, fast track, etc",
      "raw": "L1:Supply Chain product:E.g. lounge, limo, fast track, etc"
    },
    {
      "level": "L2",
      "name": "Benefit management",
      "description": "E.g. swap entitlement, split entitlement, etc",
      "raw": "L2:Benefit management:E.g. swap entitlement, split entitlement, etc"
    },
    {
      "level": "L3",
      "name": "Loyalty marketing",
      "description": "Focus on the marketing campaign or revenue burning of supply chain products (E.g. action based rewards, marketing campaign tools, benefit bundle, etc)",
      "raw": "L3:Loyalty marketing:Focus on the marketing campaign or revenue burning of supply chain products (E.g. action based rewards, marketing campaign tools, benefit bundle, etc)"
    },
    {
      "level": "L4",
      "name": "User Engagemet—— Focus on end-user interaction, activation and service (E.g. Digital Concierge, AI trip planner, etc)",
      "description": "User Engagemet—— Focus on end-user interaction, activation and service (E.g. Digital Concierge, AI trip planner, etc)",
      "raw": "L4:User Engagemet—— Focus on end-user interaction, activation and service (E.g. Digital Concierge, AI trip planner, etc)"
    }
  ],
  "channels": [
    {
      "key": "app",
      "label": "Dragonpass App"
    },
    {
      "key": "h5",
      "label": "White lable H5/SDK"
    },
    {
      "key": "web",
      "label": "White label Web"
    },
    {
      "key": "api",
      "label": "Generic API"
    },
    {
      "key": "admin",
      "label": "Admin Portal"
    },
    {
      "key": "b2b",
      "label": "B2B Portal"
    }
  ],
  "products": [
    {
      "id": "row-3",
      "row": 3,
      "level": "L1",
      "series": "Supply Chain Products",
      "seriesRaw": "L1:Supply Chain Products",
      "sku": "Lounge",
      "description": "Walk-in lounge access where users present their credentials on-site at the lounge for immediate entry — no advance booking required.",
      "playbook": "",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "Y",
          "type": "supported"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-4",
      "row": 4,
      "level": "L1",
      "series": "Supply Chain Products",
      "seriesRaw": "L1:Supply Chain Products",
      "sku": "Lounge Prebooking",
      "description": "Advance lounge reservation that guarantees availability during peak hours, enabling capacity management and traveler certainty.",
      "playbook": "",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-5",
      "row": 5,
      "level": "L1",
      "series": "Supply Chain Products",
      "seriesRaw": "L1:Supply Chain Products",
      "sku": "Premium Lounge",
      "description": "DragonPass's flagship lounge product line, delivering premium lounge experiences to high-value members",
      "playbook": "",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-6",
      "row": 6,
      "level": "L1",
      "series": "Supply Chain Products",
      "seriesRaw": "L1:Supply Chain Products",
      "sku": "Spa",
      "description": "Airport spa and massage services offered as a premium sub-type within the lounge category, providing travelers with a relaxing wellness experience during their wait.",
      "playbook": "",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-7",
      "row": 7,
      "level": "L1",
      "series": "Supply Chain Products",
      "seriesRaw": "L1:Supply Chain Products",
      "sku": "Retail",
      "description": "Airport retail product display and shopping category covering travel-related lightweight consumer goods — travelers present their DragonPass membership code to receive shopping discounts.",
      "playbook": "",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-8",
      "row": 8,
      "level": "L1",
      "series": "Supply Chain Products",
      "seriesRaw": "L1:Supply Chain Products",
      "sku": "Fast Track",
      "description": "Fast Track helps travelers bypass long and sluggish airport security lines, easing their journey and enhancing the travel experience from start to finish.",
      "playbook": "",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "Y",
          "type": "supported"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-9",
      "row": 9,
      "level": "L1",
      "series": "Supply Chain Products",
      "seriesRaw": "L1:Supply Chain Products",
      "sku": "Flight Delay",
      "description": "Auto-issued compensation benefits (lounge access, fast track, dining vouchers) for delayed passengers based on real-time flight data",
      "playbook": "",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "2026Q3",
          "type": "planned"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-10",
      "row": 10,
      "level": "L1",
      "series": "Supply Chain Products",
      "seriesRaw": "L1:Supply Chain Products",
      "sku": "Set Meal",
      "description": "Providing Global restaurant set meal. Cardholders can redeem their DragonPass membership to enjoy one set meal provided by the restaurant.",
      "playbook": "",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "2026Q3",
          "type": "planned"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "2026Q3",
          "type": "planned"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "2026Q4",
          "type": "planned"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-11",
      "row": 11,
      "level": "L1",
      "series": "Supply Chain Products",
      "seriesRaw": "L1:Supply Chain Products",
      "sku": "MPV",
      "description": "Global restaurant coupon. It provides cash rebates for cardholders. After redeeming the DragonPass membership, a fixed amount can be deducted from the total bill.",
      "playbook": "",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "2026Q3",
          "type": "planned"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "2026Q3",
          "type": "planned"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-12",
      "row": 12,
      "level": "L1",
      "series": "Supply Chain Products",
      "seriesRaw": "L1:Supply Chain Products",
      "sku": "Dinning Discount",
      "description": "Global dining discount. It offers cardholders a fixed discount on restaurant bills.",
      "playbook": "",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-13",
      "row": 13,
      "level": "L1",
      "series": "Supply Chain Products",
      "seriesRaw": "L1:Supply Chain Products",
      "sku": "Airport Transport",
      "description": "Providing transfers between airport stations and and city locations.",
      "playbook": "",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "2026Q3",
          "type": "planned"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-14",
      "row": 14,
      "level": "L1",
      "series": "Supply Chain Products",
      "seriesRaw": "L1:Supply Chain Products",
      "sku": "Esim",
      "description": "Global eSIM purchasing. Users can buy eSIM plans via Dragon Pass entitlement or PAYG to get roaming data plan in specific countries or regions.",
      "playbook": "",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "2026Q3",
          "type": "planned"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "2026Q3",
          "type": "planned"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-15",
      "row": 15,
      "level": "L1",
      "series": "Supply Chain Products",
      "seriesRaw": "L1:Supply Chain Products",
      "sku": "H&W-Fitness",
      "description": "Providing users with gym class reservations and one-time gym access. Users may book fitness class through Dragon Pass membership benefits or PAYG",
      "playbook": "",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "2026Q3",
          "type": "planned"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "2026Q3",
          "type": "planned"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-16",
      "row": 16,
      "level": "L1",
      "series": "Supply Chain Products",
      "seriesRaw": "L1:Supply Chain Products",
      "sku": "H&W-Padel",
      "description": "Global padel court booking, tournament access and training course benefits",
      "playbook": "",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "2026Q3",
          "type": "planned"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "2026Q3",
          "type": "planned"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-17",
      "row": 17,
      "level": "L1",
      "series": "Supply Chain Products",
      "seriesRaw": "L1:Supply Chain Products",
      "sku": "Local Dinning",
      "description": "Exclusive restaurant discounts, priority seating and custom packages in global popular cities, enriching local lifestyle benefit modules.",
      "playbook": "",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "2026Q3",
          "type": "planned"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "2026Q3",
          "type": "planned"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-18",
      "row": 18,
      "level": "L1",
      "series": "Supply Chain Products",
      "seriesRaw": "L1:Supply Chain Products",
      "sku": "Ticket",
      "description": "Global attraction ticket and event ticket benefit（e.g. Disney land Ticket,concert ticket,worldcup ticket ect.)",
      "playbook": "",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-19",
      "row": 19,
      "level": "L1",
      "series": "Supply Chain Products",
      "seriesRaw": "L1:Supply Chain Products",
      "sku": "Airport Parking",
      "description": "Aiport parking booking and discount",
      "playbook": "",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-20",
      "row": 20,
      "level": "L1",
      "series": "Supply Chain Products",
      "seriesRaw": "L1:Supply Chain Products",
      "sku": "Local Offer",
      "description": "",
      "playbook": "",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-21",
      "row": 21,
      "level": "L2",
      "series": "Benefit Management",
      "seriesRaw": "L2:Benefit Management",
      "sku": "Entitlement management:\nShared Entitlement",
      "description": "Supports users sharing the same entitlement pot with companions, suitable for multi-person travel scenarios.",
      "playbook": "Entitlement Overview\n平台产品能力及文档介绍汇总（中+英）\nEntitlement structure: https://dragonpass.atlassian.net/wiki/x/UABwWg\nEntitlement management\nhttps://dragonpassuk.sharepoint.com/:p:/s/SolutionManagerTeam/IQAaeza8xdKARoQ5GOJyz4idAYZBtA4QKuuMcxm4YlhrySE?e=3d8bGN",
      "playbookLinks": [
        "https://dragonpass.atlassian.net/wiki/x/UABwWg",
        "https://dragonpassuk.sharepoint.com/:p:/s/SolutionManagerTeam/IQAaeza8xdKARoQ5GOJyz4idAYZBtA4QKuuMcxm4YlhrySE?e=3d8bGN"
      ],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-22",
      "row": 22,
      "level": "L2",
      "series": "Benefit Management",
      "seriesRaw": "L2:Benefit Management",
      "sku": "Entitlement management:\nSwap entitlement - cross module entitlements",
      "description": "Supports entitlements to be used interchangeably across different modules.",
      "playbook": "Entitlement Overview\n平台产品能力及文档介绍汇总（中+英）\nEntitlement structure: https://dragonpass.atlassian.net/wiki/x/UABwWg\nEntitlement management\nhttps://dragonpassuk.sharepoint.com/:p:/s/SolutionManagerTeam/IQAaeza8xdKARoQ5GOJyz4idAYZBtA4QKuuMcxm4YlhrySE?e=3d8bGN",
      "playbookLinks": [
        "https://dragonpass.atlassian.net/wiki/x/UABwWg",
        "https://dragonpassuk.sharepoint.com/:p:/s/SolutionManagerTeam/IQAaeza8xdKARoQ5GOJyz4idAYZBtA4QKuuMcxm4YlhrySE?e=3d8bGN"
      ],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-23",
      "row": 23,
      "level": "L2",
      "series": "Benefit Management",
      "seriesRaw": "L2:Benefit Management",
      "sku": "Entitlement management:\nSwap Entitlement - Amount offset",
      "description": "user can apply number of entitlements to cover part of cost of 1 usage of a certain product, and pays for the rest.\nFor example client can offer entitlements of a certain value (e.g: US 32$) and allows users to use 1 entitlement to cover US 32$ when booking an airport transfer ride, user can apply up tp 3 entitlements in an order to cover up to US96$, and pay for the rest.",
      "playbook": "Entitlement Overview\n平台产品能力及文档介绍汇总（中+英）\nEntitlement structure: https://dragonpass.atlassian.net/wiki/x/UABwWg\nEntitlement management\nhttps://dragonpassuk.sharepoint.com/:p:/s/SolutionManagerTeam/IQAaeza8xdKARoQ5GOJyz4idAYZBtA4QKuuMcxm4YlhrySE?e=3d8bGN",
      "playbookLinks": [
        "https://dragonpass.atlassian.net/wiki/x/UABwWg",
        "https://dragonpassuk.sharepoint.com/:p:/s/SolutionManagerTeam/IQAaeza8xdKARoQ5GOJyz4idAYZBtA4QKuuMcxm4YlhrySE?e=3d8bGN"
      ],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-24",
      "row": 24,
      "level": "L2",
      "series": "Benefit Management",
      "seriesRaw": "L2:Benefit Management",
      "sku": "Entitlement management:\nSplit Entitlement",
      "description": "different funding parties funds different number of entitlements of different service networks.\nE.g: Issuer funds unlimited domestic lounges visits while Mastercard funds the oversea lounge visits.",
      "playbook": "Entitlement Overview\n平台产品能力及文档介绍汇总（中+英）\nEntitlement structure: https://dragonpass.atlassian.net/wiki/x/UABwWg\nEntitlement management\nhttps://dragonpassuk.sharepoint.com/:p:/s/SolutionManagerTeam/IQAaeza8xdKARoQ5GOJyz4idAYZBtA4QKuuMcxm4YlhrySE?e=3d8bGN",
      "playbookLinks": [
        "https://dragonpass.atlassian.net/wiki/x/UABwWg",
        "https://dragonpassuk.sharepoint.com/:p:/s/SolutionManagerTeam/IQAaeza8xdKARoQ5GOJyz4idAYZBtA4QKuuMcxm4YlhrySE?e=3d8bGN"
      ],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-25",
      "row": 25,
      "level": "L2",
      "series": "Benefit Management",
      "seriesRaw": "L2:Benefit Management",
      "sku": "Entitlement management:\nMixed Entitlement",
      "description": "Mixed entitlement allows one funding party to pay for certain entitlements and the other funding party to top up entitlements based on that.\nE.g: Mastercard funds unlimited lounge entitlements for CH, and 1 for guest. The issuer top up 2 more guest entitlements based on this. The first guest visit will be charged to Mastercard, 2 other guest visits will be charged to Issuer, other guest visits after these will be PAYG.",
      "playbook": "Entitlement Overview\n平台产品能力及文档介绍汇总（中+英）\nEntitlement structure: https://dragonpass.atlassian.net/wiki/x/UABwWg\nEntitlement management\nhttps://dragonpassuk.sharepoint.com/:p:/s/SolutionManagerTeam/IQAaeza8xdKARoQ5GOJyz4idAYZBtA4QKuuMcxm4YlhrySE?e=3d8bGN",
      "playbookLinks": [
        "https://dragonpass.atlassian.net/wiki/x/UABwWg",
        "https://dragonpassuk.sharepoint.com/:p:/s/SolutionManagerTeam/IQAaeza8xdKARoQ5GOJyz4idAYZBtA4QKuuMcxm4YlhrySE?e=3d8bGN"
      ],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-26",
      "row": 26,
      "level": "L2",
      "series": "Benefit Management",
      "seriesRaw": "L2:Benefit Management",
      "sku": "Redemption Method:\nMembership",
      "description": "Membership - User can be offered a DP membership that includes a set of entitlements of different module products. memberships has a long lifetime (e.g: 1 year, or inifinte if it never expires) and can be renewed after expired.",
      "playbook": "Entitlement management\nhttps://dragonpassuk.sharepoint.com/:p:/s/SolutionManagerTeam/IQAaeza8xdKARoQ5GOJyz4idAYZBtA4QKuuMcxm4YlhrySE?e=3d8bGN",
      "playbookLinks": [
        "https://dragonpassuk.sharepoint.com/:p:/s/SolutionManagerTeam/IQAaeza8xdKARoQ5GOJyz4idAYZBtA4QKuuMcxm4YlhrySE?e=3d8bGN"
      ],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-27",
      "row": 27,
      "level": "L2",
      "series": "Benefit Management",
      "seriesRaw": "L2:Benefit Management",
      "sku": "Redemption Method:\nVoucher",
      "description": "Vouchers - DP supports 3 different types of vouchers:\n- Percentage off voucher, e.g: 20% off\n- Fixed amount off voucher: e.g: 5USD off\n- Single use voucher: voucher that can be use to redeem 1 unit for 1 person (e.g: 1 lounge access for a person)",
      "playbook": "Entitlement management\nhttps://dragonpassuk.sharepoint.com/:p:/s/SolutionManagerTeam/IQAaeza8xdKARoQ5GOJyz4idAYZBtA4QKuuMcxm4YlhrySE?e=3d8bGN",
      "playbookLinks": [
        "https://dragonpassuk.sharepoint.com/:p:/s/SolutionManagerTeam/IQAaeza8xdKARoQ5GOJyz4idAYZBtA4QKuuMcxm4YlhrySE?e=3d8bGN"
      ],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-28",
      "row": 28,
      "level": "L2",
      "series": "Benefit Management",
      "seriesRaw": "L2:Benefit Management",
      "sku": "Redemption Method:\nePass",
      "description": "ePass - ePass is digital certificates for user to access benefits of a specific module product.",
      "playbook": "Entitlement management\nhttps://dragonpassuk.sharepoint.com/:p:/s/SolutionManagerTeam/IQAaeza8xdKARoQ5GOJyz4idAYZBtA4QKuuMcxm4YlhrySE?e=3d8bGN",
      "playbookLinks": [
        "https://dragonpassuk.sharepoint.com/:p:/s/SolutionManagerTeam/IQAaeza8xdKARoQ5GOJyz4idAYZBtA4QKuuMcxm4YlhrySE?e=3d8bGN"
      ],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "Y",
          "type": "supported"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "Y",
          "type": "supported"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-29",
      "row": 29,
      "level": "L2",
      "series": "Benefit Management",
      "seriesRaw": "L2:Benefit Management",
      "sku": "Budget Control",
      "description": "Budget control functionality controls:\n1. Issuance of entitlements\n2. Issuance of vouchers (to be developed)\nBy limiting numbers of entitlements/vouchers issued it ensure that the budget of the loyalty program or campaign doesn't overrun.",
      "playbook": "",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-30",
      "row": 30,
      "level": "L2",
      "series": "Benefit Management",
      "seriesRaw": "L2:Benefit Management",
      "sku": "Risk Control",
      "description": "Risk control functionality includes:\n1. Fair usage\n2. Dynamic QR code\n3. Location based service\n4. Boarding Pass scan",
      "playbook": "平台风控能力盘点\nhttps://dragonpassuk.sharepoint.com/:p:/s/SolutionManagerTeam/IQDn4M6QhZAKTbD6gLPmcVNEAZHpcijViIhhugcnGnzw2ik?e=MeVdmZ",
      "playbookLinks": [
        "https://dragonpassuk.sharepoint.com/:p:/s/SolutionManagerTeam/IQDn4M6QhZAKTbD6gLPmcVNEAZHpcijViIhhugcnGnzw2ik?e=MeVdmZ"
      ],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "Y",
          "type": "supported"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-31",
      "row": 31,
      "level": "L3",
      "series": "Loyalty Marketing",
      "seriesRaw": "L3:Loyalty Marketing",
      "sku": "Benefit Bundle",
      "description": "Benefit Bundle aims to leverage the platform's data capabilities to design benefit packages tailored to specific customer segments, budgets, and scenarios. By empowering user engagement through data insights, we create new revenue streams from B2C 、B2B revenue for business growth.",
      "playbook": "L3: Loyalty Marketing -  Benefit Bundle",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "2026Q3",
          "type": "planned"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "2026Q3",
          "type": "planned"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-32",
      "row": 32,
      "level": "L3",
      "series": "Loyalty Marketing",
      "seriesRaw": "L3:Loyalty Marketing",
      "sku": "Action based reward",
      "description": "A loyalty program feature where users automatically earn rewards by completing specific actions. A fully configurable backend lets you define any action and surface it to users, driving engagement and retention. Comes with an Action Library of standard modules mapped to the customer lifecycle (e.g., daily check-in, purchase milestones, referrals), making it easy to boost activity and loyalty out of the box.",
      "playbook": "L3:Loyalty Marketing- Action Base Reward",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "2026Q3",
          "type": "planned"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "2026Q3",
          "type": "planned"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-33",
      "row": 33,
      "level": "L3",
      "series": "Loyalty Marketing",
      "seriesRaw": "L3:Loyalty Marketing",
      "sku": "Tier membership Management",
      "description": "A stratified loyalty system that segments users by tier based on spend, points, or customer segement. Each tier unlocks a differentiated benefit package, automatically granted upon tier qualification. Benefits accumulate as users move up tiers, with entitlements dynamically calculated per tier level.",
      "playbook": "L3: Loyalty Marketing-Tier membership Management",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "2027Q4",
          "type": "planned"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "2027Q4",
          "type": "planned"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-34",
      "row": 34,
      "level": "L3",
      "series": "Loyalty Marketing",
      "seriesRaw": "L3:Loyalty Marketing",
      "sku": "Campaigns Management",
      "description": "Trigger campaigns from onboarding, repeat purchase, inactivity, tier movement, milestones, or expiring balances.\nPair audience, reward logic, template, and validity window in one governed workflow.",
      "playbook": "Campaign type:\nSend Promotion、Send Reward\n1:Audience / Segementation\n2:Trigger(Onboarding、Anniversary / Birthday、Points or benefit Expiration)\n3:Reward\n4:Communication (Email、SMS、Push Notification)",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "No plan",
          "type": "no-plan"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-35",
      "row": 35,
      "level": "L3",
      "series": "Loyalty Marketing",
      "seriesRaw": "L3:Loyalty Marketing",
      "sku": "Gamification",
      "description": "Design game mechanics that drive repeat behavior,Launch missions, streaks, and progression loops with guardrails that protect your reward budget and prove incrementality.",
      "playbook": "1:lucky draw (spin-the-wheel / grid lottery)\n2:Leaderboards (spend & engagement rankings)\n3:achievement badges\n4:daily check-in (streak-based tiered rewards),\n5:Seasonal challenges",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-36",
      "row": 36,
      "level": "L3",
      "series": "Loyalty Marketing",
      "seriesRaw": "L3:Loyalty Marketing",
      "sku": "Points earning and burning",
      "description": "",
      "playbook": "",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": "Staic?"
    },
    {
      "id": "row-37",
      "row": 37,
      "level": "L4",
      "series": "User Engagement",
      "seriesRaw": "L4:User Engagement",
      "sku": "BenefitQ",
      "description": "AI-powered benefits distribution engine. Designed for B2B membership benefits management, BenefitQ addresses the core challenge: how to match vast inventories of benefits to the right people with precision. Leveraging user profiles, behavioral preferences, and consumption contexts, it applies AI for intelligent benefit recommendation, dynamic pricing, and redemption guidance — ensuring every benefit delivers maximum value, boosting member engagement and utilization rates.",
      "playbook": "",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-38",
      "row": 38,
      "level": "L4",
      "series": "User Engagement",
      "seriesRaw": "L4:User Engagement",
      "sku": "AI Trip Assistant",
      "description": "Full-journey AI companion. Covering the entire travel lifecycle — pre-trip (itinerary planning, benefit reminders), in-trip (real-time flight updates, airport navigation, disruption alerts), and post-trip (expense summaries, experience feedback). It replaces traditional menu-driven interfaces with conversational interaction, allowing travelers to manage everything they need as naturally as chatting with a friend.",
      "playbook": "",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "No plan",
          "type": "no-plan"
        }
      ],
      "demo": "",
      "case": ""
    },
    {
      "id": "row-39",
      "row": 39,
      "level": "L4",
      "series": "User Engagement",
      "seriesRaw": "L4:User Engagement",
      "sku": "Digital Concierge",
      "description": "AI-powered service hub for the airport experience. Aggregates all available airport resources — lounges, fast-track, chauffeur, F&B, retail, and more — and uses natural language understanding to interpret traveler needs, then intelligently recommends and orchestrates services. Delivers hyper-personalized, \"one traveler, one journey\" experiences. This is fundamentally the L4-layer AI service front-end.",
      "playbook": "",
      "playbookLinks": [],
      "support": [
        {
          "key": "app",
          "label": "Dragonpass App",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "h5",
          "label": "White lable H5/SDK",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "web",
          "label": "White label Web",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "api",
          "label": "Generic API",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "admin",
          "label": "Admin Portal",
          "value": "",
          "type": "unknown"
        },
        {
          "key": "b2b",
          "label": "B2B Portal",
          "value": "",
          "type": "unknown"
        }
      ],
      "demo": "",
      "case": ""
    }
  ]
};
